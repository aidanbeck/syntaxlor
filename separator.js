function getSymbolIndexes(string, syntax) {
    ...
    for (let index = 1; index < string.length; index++) { // for each character
        for (let rule of syntax.rules) {                            // for each rule
            if (string.charAt(index) == rule.symbol) {    //if the character == the rule's symbol (and isn't ignored)
                ...
                indexes.push(index);
                // !!! if commentSymbol, don't look for more symbols.
            }
        }
    }
    return indexes;
...
function getCommand(statement, syntax) {
    for (let rule of syntax.rules) {
        if (statement.charAt(0) == rule.symbol) {
            ...
            return {commandFunction: rule.ruleFunction, value: value }; // could instead safe the index of each rule?
        }
        if (statement.charAt(0) == syntax.commentSymbol) { //comments
            return {commandFunction: () => {}, value: ""}; // empty command
        }
    }
    return {commandFunction: syntax.defaultFunction, value: statement.replace('`', '').trim() }; // default/no symbol rule
}
